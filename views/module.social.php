...
if(!empty($icon)){
    $disicon=' <a href="' . $socialRow->linksrc . '" target="_blank" rel="noopener"><img src="'.IMAGE_PATH.'social/' . $socialRow->image . '"  width="25px" alt="' . $socialRow->title . '"/></a> &nbsp;';
}
else{
    $disicon='<a href="' . $socialRow->linksrc . '" target="_blank" rel="noopener"><i class="' . $socialRow->icon . '" aria-hidden="true"></i></a> &nbsp;';
}
...
$otaRec = SocialNetworking::getotaNetwork();
$ota = '';
if (!empty($otaRec)) {
    foreach ($otaRec as $otaRow) {
        $ota .= ' 
        <div class="ota_img"><a href="' . $otaRow->linksrc . '" target="_blank" rel="noopener"><img src="'.IMAGE_PATH.'social/' . $otaRow->image . '" alt="' . $otaRow->title . '"></a></div>';
    }
}
