...
class Article extends DatabaseObject {
	...
	protected static $db_fields = array('id', 'parent_id', 'slug', 'title', 'sub_title', 'linksrc', 'linktype', 'content', 'status', 'sortorder', 'meta_title', 'meta_keywords', 'meta_description', 'type', 'added_date', 'modified_date','homepage', 'image', 'image_gal');
	...
	public static function find_by_id_home($id=0){
		global $db;
		$result_array = self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE id={$id} AND homepage='1' LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
	}
