...
static function getAllImg(){
		...
		return self::find_by_sql("SELECT c.* FROM " . self::$table_name . " AS c 
                INNER JOIN tbl_galleries as p
                WHERE p.id= c.galleryid AND p.type =1 AND c.status=1 AND p.status=1 
