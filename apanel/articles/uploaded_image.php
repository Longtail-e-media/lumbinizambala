...
if(!empty($imageNameArr)):
foreach($imageNameArr as $key=>$val):?>
<div class="col-md-3" id="previewUserimagegal<?php echo $deleteid;?>">
    <div class="infobox info-bg">
        <img src="<?php echo IMAGE_PATH.'articles/gallery/thumbnails/'.$val;?>"  style="width:100%"/>
        <a href="javascript:void(0);" onclick="deleteTempimagegal(<?php echo $deleteid;?>);">
            <span class="badge badge-absolute float-right bg-red" style="right: -10px !important;">
                <i class="glyph-icon icon-clock-os"></i>
            </span>
        </a>
...
//uplodify
  if(isset($_SESSION['imageNameArr'])){
   if(count($_SESSION['imageNameArr'])>0){
      foreach($_SESSION['imageNameArr'] as $key=>$val)
      { 
         @unlink(IMAGE_PATH.'articles/gallery/thumbnails/'.$val);
      	 @unlink(IMAGE_PATH.'articles/gallery/'.$val);
      }	 
    	unset($_SESSION['imageNameArr']);
       }
  }
