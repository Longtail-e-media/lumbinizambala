...
<?php elseif (isset($_GET['mode']) && $_GET['mode'] == "addEdit"):
        ...
        <div class="example-code">
            <form action="" class="col-md-12 center-margin" id="articles_frm">
                ...
                <div class="form-row add-image">
                    ...
                    <div class="form-input col-md-10 uploader">
                        <input type="file" name="gallery_uploadgal" id="gallery_uploadgal" class="transparent no-shadow">
                        <label>
                            <small>Image Dimensions (1920 px X 438 px)</small>
                        </label>
                    </div>
                    <!-- Upload user image preview -->
                    <div id="preview_Imagegal"><input type="hidden" name="imageArrayname1[]"/></div>
                    <?php
                    if (!empty($articlesInfo->image_gal)) {
                        $imgRec = unserialize($articlesInfo->image_gal);
                        if (is_array($imgRec)) {
                            foreach ($imgRec as $key => $recimg) {
                                $deleteid = rand(0, 99999);
                                $imagePath = SITE_ROOT . 'images/articles/gallery/' . $recimg;
                                if (file_exists($imagePath)) { ?>
                                    <div class="col-md-3" id="removeSavedimggal<?php echo $deleteid; ?>">
                                        <div class="infobox info-bg">
                                            <div class="button-group" data-toggle="buttons">
                            <span class="float-left">
                                <?php
                                if (file_exists(SITE_ROOT . "images/articles/gallery/" . $recimg)):
                                    $filesize = filesize(SITE_ROOT . "images/articles/gallery/" . $recimg);
                                    echo 'Size : ' . getFileFormattedSize($filesize);
                                endif;
                                ?>
                            </span>
                                                <a class="btn small float-right" href="javascript:void(0);"
                                                   onclick="deleteSavedArticlesimagegal(<?php echo $deleteid; ?>);">
                                                    <i class="glyph-icon icon-trash-o"></i>
                                                </a>
                                            </div>
                                            <img src="<?php echo IMAGE_PATH . 'articles/gallery/thumbnails/' . $recimg; ?>"
                                                 style="width:100%"/>
    ...
    <script>
        $(document).ready(function () {
            $('#gallery_uploadgal').uploadify({
                'swf': '<?php echo ASSETS_PATH;?>uploadify/uploadify.swf',
                'uploader': '<?php echo ASSETS_PATH;?>uploadify/uploadify.php',
                'formData': {
                    PROJECT: '<?php echo SITE_FOLDER;?>',
                    targetFolder: 'images/articles/gallery/',
                    thumb_width: 200,
                    thumb_height: 200
                },
                'method': 'post',
                ...
                'onUploadSuccess': function (file, data, response) {
                    $('#uploadedImageNamea').val('1');
                    var filename = data;
                    $.post('<?php echo BASE_URL;?>apanel/articles/uploaded_image.php', {imagefile: filename, action: 'multiple'}, function (msg) {
                        $('#preview_Imagegal').append(msg).show();
                    });
        ...
        $(document).ready(function () {
            $('#gallery_upload').uploadify({
                ...
                'onUploadSuccess': function (file, data, response) {
                    ...
                    var filename = data;
                    $.post('<?php echo BASE_URL;?>apanel/articles/uploaded_image.php', {imagefile: filename, action: 'single'}, function (msg) {
